...
import { reportedBugsIndividualData } from "../../helper/Table/ReportedBugsHelper";
import { updateReportedBug } from "../../helper/Table/ReportedBugsHelper";
import { toast } from "react-toastify";

const ReportedBugDetail = () => {
  // Context
  ...
  // Destructure
  const { firstName, lastName, title, Status, description, createdAt } = values;
  const { status, Description, image, Title } = updatedValues;

  const preload = () => {
    console.log(id, "id");
    reportedBugsIndividualData(id, data.accessToken)
      .then((data) => {
        ...
        if (data.message == "success") {
          setValues({
            ...values,
            firstName: data.data.Details.user.firstName,
            lastName: data.data.Details.user.lastName,
  ...
  const onSubmit = (event) => {
    ...
    updateReportedBug(id, data.accessToken, updatedValues).then((data) => {
      console.log(data);
      if (data.message == "success") {
        setUpdatedValues({
          ...updatedValues,
  ...
  return (
    ...
    >
      <Link to="/reportedBugs">
        <IoIosArrowBack
          size={25}
          className={`mb-3 ${currentMode === "Dark" ? "text-white" : ""}`}
        />
      </Link>
      <Header category="Details" title="Reported Bug Details" />
...
export default ReportedBugDetail;
